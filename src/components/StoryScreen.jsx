  ...
  if (done) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6 px-6 bg-teal-light/30" dir={isEn ? 'ltr' : 'rtl'}>
        ...
        <h2 className="font-display text-3xl text-center text-teal-dark">{isEn ? `You finished ${storyTitle}!` : `خلصت قصة ${storyTitle}!`}</h2>
        <button onClick={handleBack} className="bg-teal text-white font-body font-bold rounded-full px-8 py-3 shadow-lg active:scale-95 transition-transform">{isEn ? 'Back to stories' : 'رجوع للقصص'}</button>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-teal-light/20" dir={isEn ? 'ltr' : 'rtl'}>
      <div className="flex items-center justify-between px-6 py-4">
        <button onClick={handleBack} className="font-body font-bold text-ink bg-white/60 rounded-full px-4 py-2">{isEn ? 'Back' : 'رجوع'}</button>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 gap-6">
        ...
        <div className="bg-white rounded-3xl shadow-xl px-6 py-8 w-full max-w-sm">
          ...
          <button onClick={() => playText(currentText, lang)} className="mx-auto mt-5 flex items-center justify-center w-14 h-14 rounded-full bg-teal text-white shadow-md active:scale-90 transition-transform" aria-label={isEn ? 'Listen to story' : 'اسمع القصة'}>🔊</button>
        </div>
      </div>

      <div className="px-6 pb-10">
        <button onClick={handleNext} className="w-full bg-teal text-white font-display text-xl rounded-full py-4 shadow-lg active:scale-95 transition-transform">{isLast ? (isEn ? 'Finish the story! 🎉' : 'خلصت القصة! 🎉') : (isEn ? 'Next ➡️' : 'بعدين ⬅️')}</button>
      </div>
    </div>
  )
}

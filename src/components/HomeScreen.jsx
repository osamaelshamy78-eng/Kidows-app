export default function HomeScreen({ lang, onToggleLanguage, onSelect, stars }) {
  const hour = new Date().getHours(); const isDayTime = hour >= 5 && hour < 18; const isEn = lang === 'en'
  return <div className="min-h-screen flex flex-col items-center justify-between bg-cream px-6 py-8" dir={isEn ? 'ltr' : 'rtl'}>
    <div className="w-full flex items-center justify-between gap-3"><div className="flex items-center gap-1 bg-white/70 rounded-full px-4 py-2 shadow-sm"><span className="text-xl">⭐</span><span className="font-body font-bold text-ink">{stars}</span></div><h1 className="font-display text-3xl text-teal-dark text-center flex-1">{isEn ? 'Little Garden' : 'روضة الصبيان'}</h1><button onClick={onToggleLanguage} className="bg-white/80 rounded-full px-3 py-2 font-body font-bold text-teal-dark shadow-sm">{isEn ? 'عربي' : 'English'}</button></div>
    ...
    <div className="w-full flex flex-col gap-4">
      ...
      <button onClick={()=>onSelect('games')} className="w-full bg-teal-dark rounded-blob py-6 px-6 flex items-center justify-between shadow-lg"><span className="text-4xl">🎨</span><span className="font-display text-2xl text-white">{isEn?'Fun Games':'ألعاب ممتعة'}</span></button>
    </div>
